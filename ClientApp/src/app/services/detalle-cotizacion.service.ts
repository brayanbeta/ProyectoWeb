...
import { DetalleCotizacion } from '../Proyecto/models/detalle-cotizacion';
...
export class DetalleCotizacionService {
  ...
  update(detalleCotizacion: DetalleCotizacion): Observable<any> {
    const url = `${this.baseUrl + 'api/detalleCotizacion'}/${detalleCotizacion.codigoCotizacion}`;
    return this.http.put(url, detalleCotizacion, httpOptions).pipe(
      tap(_ => this.log(`se modificaron los datos del detalleCotizacion con id=${detalleCotizacion.codigoCotizacion}`)),
  ...
  delete(detalleCotizacion: DetalleCotizacion | string): Observable<DetalleCotizacion> {
    const id = typeof detalleCotizacion === 'string' ? detalleCotizacion : detalleCotizacion.codigoCotizacion;
    const url = `${this.baseUrl + 'api/detalleCotizacion'}/${id}`;
